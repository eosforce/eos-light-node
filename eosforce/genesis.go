...
type Genesis struct {
	...
	InitialAccountList []struct {
		Key   string `json:"key"`
		Asset string `json:"asset"`
		Name  string `json:"name"`
	} `json:"initial_account_list"`
	InitialProducerList []struct {
		Name           string `json:"name"`
		Bpkey          string `json:"bpkey"`
		CommissionRate int    `json:"commission_rate"`
		URL            string `json:"url"`
	} `json:"initial_producer_list"`
}

// NewGenesisFromFile new genesis from file
func NewGenesisFromFile(path string) (*Genesis, error) {
	res := &Genesis{}
	err := LoadJSONFile(path, res)
	if err != nil {
		return nil, err
	}
